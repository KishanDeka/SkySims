...
import os,sys
sys.path.append('taylens/')
from taylens import *

# This generates correlated T,E,B and Phi maps
def simulate_tebp_correlated(cl_tebp_arr,nside,lmax,pol) :
...
#Set to zero above map resolution to avoid aliasing
	...
	cmb=np.array(hp.alm2map(acmb,nside,pol=pol))
...
parser.add_argument("-N", "--nside",   type=int, default=512, help="Simulation HEALPix Nside.")
...
parser.add_argument("-p", "--pol",     action="store_false",   help="If specified, simulations will be full TEB-simulations rather than just T.")
...
prt("Computing ipos")
ipos = np.array(hp.pix2ang(args.nside, np.arange(hp.nside2npix(args.nside))))
...
for sim in range(myid, args.nsim, nproc):
	...
	cmb, aphi = simulate_tebp_correlated(cl_tebp_arr,args.nside,synlmax,args.pol)

	if "u" in args.output:
		prt("Writing unlensed map")
		hp.write_map("%s/ucmb%03d_%d.fits" % (args.odir, sim, args.order),
				 cmb)
	if cmb.ndim == 1: 
	...
	phi, phi_dtheta, phi_dphi = hp.alm2map_der1(aphi, args.nside, lmax=synlmax)
	if "p" in args.output:
		prt("Writing lensing field")
		hp.write_map("%s/phi%03d_%d.fits" % (args.odir, sim, args.order), phi)
	if "g" in args.output:
		prt("Writing gradient")
		hp.write_map("%s/grad%03d_%d.fits" % (args.odir, sim, args.order), [phi_dtheta, phi_dphi])
	del aphi
	...
	if "l" in args.output:
		...
		hp.write_map("%s/lcmb%03d_%d.fits" % (args.odir, sim, args.order), rm)
	if "s" in args.output:
		prt("Computing power spectrum")
		alm  = hp.map2alm(rm, use_weights=True, iter=1)
		spec = np.array(hp.alm2cl(alm))
